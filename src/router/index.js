...
router.get("/news/[0-9]+", async (ctx) => {
  const newsId = ctx.request.url.split("/")[2];
...
router.get("/^/profile/[a-zA-Z0-9]+$/", async (ctx) => {
  const username = ctx.request.url.split("/")[2];
  console.log("profile");
...
router.post("/comment/[0-9]+", async (ctx) => {
  const commentId = ctx.request.url.split("/")[2];
  return await formController.comment(ctx, commentId);
});

router.post("/uploadnews", async (ctx) => await formController.uploadNews(ctx));
